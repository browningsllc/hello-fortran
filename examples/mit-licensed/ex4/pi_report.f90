!-----------------------------------------------------------------------------
! File Name:  pi_report.f90
! Purpose:    Approximate pi from a namelist file named on the command line
!             (as in ex3), then write the results to an ASCII report file.
!             The report file is named in the namelist and is overwritten if
!             it already exists.  Every &pi_input group in the file is read
!             and run in turn, and all of them share one report file.
!
...
!
! Build:      ./build.sh
! Run:        ./pi_report.exe pi_input.nml
! Clean:      ./cl.sh
!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------
! report_mod -- write the results to an ASCII file.  Depends on pi_mod for
! the reference value, so the module chain is kind_mod -> pi_mod -> report_mod
! and the file is laid out in that order.
!-----------------------------------------------------------------------------
module report_mod
  use kind_mod, only: ik, rk
  use pi_mod,   only: pi_ref, pi_error
  implicit none (type, external)
  private
  public :: write_report
  ...
  !> NOT pure -- external file I/O is forbidden in a pure procedure.
  !>
  !> One report file holds every case, so this is called once per case and
  !> opens the file two different ways:
  !>
  !>   first case  -> status='replace': create the file, or truncate an old
  !>                  one.  Run the program twice and the report holds one set
  !>                  of results, never two.
  !>   later cases -> position='append': add to what is already there.
  !>
  !> That pair is the practical distinction to remember.  status='unknown'
  !> leaves the choice to the processor, which is why we never use it.
  subroutine write_report(out_file, n_case, n_total, method, n, pi_est, stat, msg)
    character(len=*), intent(in)  :: out_file, method
    integer(ik),      intent(in)  :: n_case, n_total, n
    real(rk),         intent(in)  :: pi_est
    ...
    if (n_case == 1_ik) then
      ...
      open (newunit=unit, file=out_file, status='old', position='append', &
    ...
    ! The banner belongs to the file, not to a case, so write it once.
    if (n_case == 1_ik) then
      write (unit, '(a)') ' Hello Fortran -- pi approximation'
      write (unit, '(a)') ' ================================='
    end if

    write (unit, '(a)')             ''
    write (unit, '(a, i0, a, i0)')  ' case       : ', n_case, ' of ', n_total
    write (unit, '(a)')             ' ---------------------------------'
    write (unit, '(a, a)')          ' method     : ', trim(method)
    write (unit, '(a, i0)')         ' n          : ', n
    write (unit, '(a, f0.12)')      ' pi (est)   : ', pi_est
    write (unit, '(a, f0.12)')      ' pi (ref)   : ', pi_ref
    write (unit, '(a, es10.3)')     ' abs error  : ', pi_error(pi_est)

    close (unit, iostat=ios, iomsg=msg)
    if (ios /= 0) stat = ios
  end subroutine write_report
end module report_mod

!-----------------------------------------------------------------------------
! Main program: read the namelist, approximate pi, print to the screen AND
! write an ASCII report file.
!-----------------------------------------------------------------------------
program pi_report
  ...
  use report_mod, only: write_report
  implicit none (type, external)

  ! As in ex3, every namelist member has a default.  out_file is the new one:
  ! leave it out of the input file and the report still lands somewhere sane.
  character(len=32)  :: method   = 'midpoint'
  integer(ik)        :: n        = 1000_ik
  character(len=128) :: out_file = 'pi_report.out'

  namelist /pi_input/ method, n, out_file
  ...
  if (command_argument_count() /= 1) then
    write (error_unit, '(a)') 'usage:   ./pi_report.exe <namelist file>'
    write (error_unit, '(a)') 'example: ./pi_report.exe pi_input.nml'
  ...
  do n_case = 1_ik, n_groups
    ...
    ! --- 6. and to this group's own file ----------------------------------
    call write_report(trim(out_file), n_case, n_groups, method, n, pi_est, stat, msg)
    if (stat /= 0) error stop 'cannot write report: ' // trim(msg)
  end do

  close (unit)

  ! One report holds every case, so say so once rather than per case.
  write (*, '(a)')           ''
  write (*, '(a, i0, a, a)') ' wrote      : ', n_groups, ' case(s) to ', trim(out_file)
end program pi_report
