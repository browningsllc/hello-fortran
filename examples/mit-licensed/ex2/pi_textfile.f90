!-----------------------------------------------------------------------------
! File Name:  pi_textfile.f90
! Purpose:    Approximate pi using values read from a plain-text input file
!             whose name is supplied as a command-line argument.  The file is
!             first passed through a comment stripper that writes a cleaned
!             copy to disk; the cleaned copy is then reopened and read.
!             Demonstrates the split between pure text logic and the impure
!             I/O that surrounds it.
!
! Course:     Hello Fortran: A Modern Fortran Primer for Engineers and
!             Scientists
! Author:     Robert S. Browning
! Date:       02SEP2026
!
! SPDX-License-Identifier: MIT
! Copyright (c) 2026 Robert S. Browning
! Released under the MIT License.  See ../LICENSE for the full text.
!
! Build:      ./build.sh
! Run:        ./pi_textfile.exe pi_input.txt
! Clean:      ./cl.sh
!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
! kind_mod -- the single place the project's integer and real kinds are named.
! Every other unit takes its kinds from here, so changing precision is a
! one-line edit instead of a repo-wide search.
!-----------------------------------------------------------------------------
module kind_mod
  use iso_fortran_env, only: int32, real64
  implicit none (type, external)
  private
  integer, parameter, public :: ik = int32    ! integer kind
  integer, parameter, public :: rk = real64   ! real kind
end module kind_mod

!-----------------------------------------------------------------------------
! pi_mod -- the whole computation, and every procedure in it is pure.
! Nothing here reads, writes, or prints, so it can be reasoned about and
! reused without worrying about side effects.  The same module appears
! unchanged in all four examples.
!-----------------------------------------------------------------------------
module pi_mod
  use kind_mod, only: ik, rk
  implicit none (type, external)
  private
  public :: pi_ref, n_max, pi_leibniz, pi_midpoint, pi_estimate, pi_error

  ! The reference value is computed by the compiler, not transcribed from a
  ! book.  That is what lets every example check its own answer.
  real(rk), parameter :: pi_ref = 4.0_rk * atan(1.0_rk)

  ! Upper bound on the term count.  Keeps 2*k+1 inside the range of ik and
  ! keeps a live demo from appearing to hang.
  integer(ik), parameter :: n_max = 100000000_ik

  contains

  !> Leibniz series:  pi = 4 * sum over k of (-1)**k / (2k+1).
  !> Simple to read, and painfully slow -- the error falls only as 1/n.
  pure function pi_leibniz(n_terms) result(pi_est)
    integer(ik), intent(in) :: n_terms
    real(rk) :: pi_est

    real(rk)    :: total, sign_k
    integer(ik) :: k

    total  = 0.0_rk
    sign_k = 1.0_rk   ! toggled rather than (-1)**k: cheaper, and cannot overflow
    do k = 0_ik, n_terms - 1_ik
      total  = total + sign_k / real(2_ik*k + 1_ik, rk)
      sign_k = -sign_k
    end do
    pi_est = 4.0_rk * total
  end function pi_leibniz

  !> Midpoint rule on the integral of 4/(1+x**2) from 0 to 1, which is pi.
  !> The error falls as 1/n**2, so it reaches machine precision far sooner.
  pure function pi_midpoint(n_panels) result(pi_est)
    integer(ik), intent(in) :: n_panels
    real(rk) :: pi_est

    real(rk)    :: h, x_mid, total
    integer(ik) :: i

    h     = 1.0_rk / real(n_panels, rk)
    total = 0.0_rk
    do i = 1_ik, n_panels
      x_mid = (real(i, rk) - 0.5_rk) * h        ! centre of panel i
      total = total + 4.0_rk / (1.0_rk + x_mid*x_mid)
    end do
    pi_est = h * total
  end function pi_midpoint

  !> Validate the input and dispatch on the method name.
  !>
  !> This is a pure SUBROUTINE rather than a pure function on purpose: a pure
  !> function may not have intent(out) dummy arguments, and this has to hand
  !> back a value, a status, and a reason.  Note that the internal write below
  !> is legal here -- the restriction on pure procedures covers external file
  !> I/O, not writing into a character variable.
  pure subroutine pi_estimate(method, n, pi_est, ok, why)
    character(len=*), intent(in)  :: method
    integer(ik),      intent(in)  :: n
    real(rk),         intent(out) :: pi_est
    logical,          intent(out) :: ok
    character(len=*), intent(out) :: why

    pi_est = 0.0_rk
    ok     = .false.
    why    = ''

    if (n < 1_ik) then
      why = 'n must be 1 or greater'
      return
    end if

    if (n > n_max) then
      write (why, '(a, i0)') 'n exceeds the supported maximum of ', n_max
      return
    end if

    select case (trim(adjustl(method)))
    case ('midpoint')
      pi_est = pi_midpoint(n)
      ok     = .true.
    case ('leibniz')
      pi_est = pi_leibniz(n)
      ok     = .true.
    case default
      why = 'unknown method "' // trim(adjustl(method)) // &
            '" (expected midpoint or leibniz)'
    end select
  end subroutine pi_estimate

  !> Absolute error against the reference.  This is the self-check: the
  !> program reports how wrong it is without being told the answer.
  pure function pi_error(pi_est) result(err)
    real(rk), intent(in) :: pi_est
    real(rk) :: err

    err = abs(pi_est - pi_ref)
  end function pi_error
end module pi_mod

!-----------------------------------------------------------------------------
! text_util_mod -- reading a commented input file, in two clearly separated
! halves.  The text logic is pure; only the file handling is not, and the one
! impure procedure says so in its own comment.  That boundary is the point of
! this module.
!-----------------------------------------------------------------------------
module text_util_mod
  use kind_mod, only: ik
  implicit none (type, external)
  private
  public :: strip_comment, clean_file_name, write_clean_copy

  character,   parameter :: comment_char = '!'
  integer(ik), parameter :: max_line     = 512_ik

  contains

  !> Pure: everything ahead of the first '!', with trailing blanks removed.
  !> A line that is entirely a comment comes back as a zero-length string.
  pure function strip_comment(line_in) result(line_out)
    character(len=*), intent(in)  :: line_in
    character(len=:), allocatable :: line_out

    integer(ik) :: pos

    pos = index(line_in, comment_char)
    if (pos > 0_ik) then
      line_out = trim(line_in(1:pos-1_ik))
    else
      line_out = trim(line_in)
    end if
  end function strip_comment

  !> Pure: swap the extension for '.clean', so pi_input.txt becomes
  !> pi_input.clean.  back=.true. finds the LAST dot, which is what you want
  !> for a name like archive.tar.txt.
  pure function clean_file_name(file_name) result(name_out)
    character(len=*), intent(in)  :: file_name
    character(len=:), allocatable :: name_out

    integer(ik) :: dot

    dot = index(file_name, '.', back=.true.)
    if (dot > 1_ik) then
      name_out = file_name(1:dot-1_ik) // '.clean'
    else
      name_out = trim(file_name) // '.clean'      ! no extension to replace
    end if
  end function clean_file_name

  !> NOT pure, and it cannot be: Fortran 2018 forbids external file I/O
  !> inside a pure procedure.  Every decision it makes comes from the pure
  !> strip_comment above; only the opening, reading, and writing live here.
  !> Isolating I/O this way is worth doing even when purity is not the goal.
  subroutine write_clean_copy(in_file, out_file, stat, msg)
    character(len=*), intent(in)  :: in_file, out_file
    integer(ik),      intent(out) :: stat
    character(len=*), intent(out) :: msg

    character(len=max_line)       :: line
    character(len=:), allocatable :: stripped
    integer(ik)                   :: unit_in, unit_out, ios

    stat = 0_ik
    msg  = ''

    open (newunit=unit_in, file=in_file, status='old', action='read', &
          iostat=ios, iomsg=msg)
    if (ios /= 0) then
      stat = ios
      return
    end if

    ! status='replace' -- create it, or truncate and overwrite an old copy.
    open (newunit=unit_out, file=out_file, status='replace', action='write', &
          iostat=ios, iomsg=msg)
    if (ios /= 0) then
      stat = ios
      close (unit_in)
      return
    end if

    do
      read (unit_in, '(a)', iostat=ios) line
      if (ios /= 0) exit                      ! end of file, or a read error
      stripped = strip_comment(line)
      if (len(stripped) > 0_ik) write (unit_out, '(a)') stripped
    end do

    close (unit_in)
    close (unit_out)
  end subroutine write_clean_copy
end module text_util_mod

!-----------------------------------------------------------------------------
! Main program: strip the comments out of the named input file, read the
! cleaned copy, approximate pi, report.
!-----------------------------------------------------------------------------
program pi_textfile
  use iso_fortran_env, only: error_unit
  use kind_mod,      only: ik, rk
  use pi_mod,        only: pi_ref, pi_estimate, pi_error
  use text_util_mod, only: clean_file_name, write_clean_copy
  implicit none (type, external)

  character(len=:), allocatable :: in_file, clean_file
  character(len=64)             :: method
  character(len=80)             :: why, msg
  integer(ik)                   :: n, unit, ios, arg_len, stat
  real(rk)                      :: pi_est
  logical                       :: ok

  ! --- 1. one argument: the name of the input file -------------------------
  if (command_argument_count() /= 1) then
    write (error_unit, '(a)') 'usage:   ./pi_textfile.exe <input file>'
    write (error_unit, '(a)') 'example: ./pi_textfile.exe pi_input.txt'
    ! error_unit is buffered when stderr is redirected to a file, and the
    ! runtime writes the ERROR STOP text straight to the file descriptor,
    ! bypassing that buffer.  Without this flush the lines above are emitted
    ! last and appear *after* the backtrace in a redirected log.
    flush (error_unit)
    error stop 'expected exactly 1 command-line argument'
  end if

  call get_command_argument(1, length=arg_len, status=stat)
  if (stat /= 0) error stop 'cannot read command-line argument 1'
  allocate (character(len=arg_len) :: in_file)
  call get_command_argument(1, value=in_file, status=stat)
  if (stat /= 0) error stop 'cannot read command-line argument 1'

  ! --- 2. write a comment-free copy, then read that ------------------------
  ! Keeping the cleaned file on disk (rather than using status='scratch')
  ! means you can cat it afterwards and see exactly what the stripper did.
  clean_file = clean_file_name(in_file)
  call write_clean_copy(in_file, clean_file, stat, msg)
  if (stat /= 0) error stop 'cannot prepare input: ' // trim(msg)

  open (newunit=unit, file=clean_file, status='old', action='read', &
        iostat=ios, iomsg=msg)
  if (ios /= 0) error stop 'cannot open ' // clean_file // ': ' // trim(msg)

  read (unit, '(a)', iostat=ios) method       ! line 1: the method name
  if (ios /= 0) error stop 'cannot read the method from ' // clean_file

  read (unit, *, iostat=ios) n                ! line 2: the term count
  if (ios /= 0) error stop 'cannot read n from ' // clean_file

  close (unit)

  ! --- 3. echo the input so the audience can confirm the read --------------
  write (*, '(a)')     ' Hello Fortran -- pi approximation'
  write (*, '(a)')     ' ---------------------------------'
  write (*, '(a, a)')  ' source     : text file ', in_file
  write (*, '(a, a)')  ' stripped to: ', clean_file
  write (*, '(a, a)')  ' method     : ', trim(method)
  write (*, '(a, i0)') ' n          : ', n

  ! --- 4. compute ----------------------------------------------------------
  call pi_estimate(method, n, pi_est, ok, why)
  if (.not. ok) error stop 'bad input: ' // trim(why)

  ! --- 5. report -----------------------------------------------------------
  write (*, '(a)')         ' ---------------------------------'
  write (*, '(a, f0.12)')  ' pi (est)   : ', pi_est
  write (*, '(a, f0.12)')  ' pi (ref)   : ', pi_ref
  write (*, '(a, es10.3)') ' abs error  : ', pi_error(pi_est)
end program pi_textfile
